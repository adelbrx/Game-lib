...
def get_random_creature_name():
    ...
    if get_nb_defeated() == 0 or random.randint(0, 2) == 0:
        ...
        prefix = ('Herbie', 'Dors', 'Giskard')[random.randint(0, 2)]
...
def is_there_a_creature():
    """Tells wheather there is already exists a creature
    
    Returns
    -------
    result : True if a creature already exists, False otherwise (bool)
    
    """    
    game_db = _load_game_db()

    return len(game_db['creatures'])>0
